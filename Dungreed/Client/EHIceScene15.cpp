...
namespace EH
{
	void IceScene15::Initialize()
	{
        ...
        SetSize(Math::Vector2<float>(1536.f, 832.f));
        ...
        IceBG1->GetComponent<Transform>()->SetPos(Math::Vector2<float>(768.f, 416.f));
        IceBG1->GetComponent<Transform>()->SetScale(Math::Vector2<float>(1536.f, 832.f));
        texture = Resources::Load<Texture>(L"IceBG15", L"..\\Resources\\Dungeon\\IceBackGround\\15\\IceScene15.bmp");
        IceBG1->GetComponent<SpriteRenderer>()->SetImg(texture);
        IceBG1->GetComponent<SpriteRenderer>()->SetAffectCamera(true);
	}

	void IceScene15::Update()
	{
		Scene::Update();
	}

	void IceScene15::Render(HDC hdc)
	{
		Scene::Render(hdc);
	}
}