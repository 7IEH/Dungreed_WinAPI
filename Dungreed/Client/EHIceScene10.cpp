...
namespace EH
{
	void IceScene10::Initialize()
	{
        ...
        SetSize(Math::Vector2<float>(2496.f, 768.f));
        ...
        IceBG1->GetComponent<Transform>()->SetPos(Math::Vector2<float>(1248.f, 384.f));
        IceBG1->GetComponent<Transform>()->SetScale(Math::Vector2<float>(2496.f, 768.f));
        texture = Resources::Load<Texture>(L"IceBG10", L"..\\Resources\\Dungeon\\IceBackGround\\10\\IceScene10.bmp");
        IceBG1->GetComponent<SpriteRenderer>()->SetImg(texture);
        IceBG1->GetComponent<SpriteRenderer>()->SetAffectCamera(true);
	}

	void IceScene10::Update()
	{
		Scene::Update();
	}

	void IceScene10::Render(HDC hdc)
	{
		Scene::Render(hdc);
	}
}