...
namespace EH
{
	void IceScene19::Initialize()
	{
        ...
        SetSize(Math::Vector2<float>(1280.f, 1536.f));
        ...
        IceBG1->GetComponent<Transform>()->SetPos(Math::Vector2<float>(640.f, 768.f));
        IceBG1->GetComponent<Transform>()->SetScale(Math::Vector2<float>(1280.f, 1536.f));
        texture = Resources::Load<Texture>(L"IceBG19", L"..\\Resources\\Dungeon\\IceBackGround\\19\\IceScene19.bmp");
        IceBG1->GetComponent<SpriteRenderer>()->SetImg(texture);
        IceBG1->GetComponent<SpriteRenderer>()->SetAffectCamera(true);   
	}

	void IceScene19::Update()
	{
		Scene::Update();
	}

	void IceScene19::Render(HDC hdc)
	{
		Scene::Render(hdc);
	}
}